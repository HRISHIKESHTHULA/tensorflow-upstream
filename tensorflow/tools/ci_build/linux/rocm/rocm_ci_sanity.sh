...
# List changed files for this branch relative to the parent branch.
# The parent branch is assumed to be origin/develop-upstream.
get_changed_files_for_this_branch() {
  git diff --name-only origin/develop-upstream --
}
...
get_py_files_to_check() {
  if [[ "$1" == "--incremental" ]]; then
    CHANGED_PY_FILES=$(get_changed_files_for_this_branch | \
                       grep '.*\.py$')
